...
lookup_test() ->
    ...
    Map = #{a => 1, b => 2},
    ?assertMatch({value, #{a => 1, b => 2}}, klsn_map:lookup([], Map)),

    %% Test with a valid single-level key
    ?assertMatch({value, 1}, klsn_map:lookup([a], Map)),

    %% Test with a nested key
    NestedMap = #{c => #{d => 4}},
    ComplexMap = #{a => 1, b => 2, c => NestedMap},
    ?assertMatch({value, 4}, klsn_map:lookup([c, d], ComplexMap)),

    %% Test with a non-existing key
    ?assertMatch(none, klsn_map:lookup([e], Map)),

    %% Test with a nested non-existing key
    ?assertMatch(none, klsn_map:lookup([c, e], ComplexMap)).
